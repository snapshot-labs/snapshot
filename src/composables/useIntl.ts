...
export function useIntl() {
  ...
  const getRelativeTimeFormatter = (options?: object) => computed(() => new Intl.RelativeTimeFormat(
    currentLocale.value,
    options || { style: 'short', numeric: 'always'}
  ));

  const getNumberFormatter = (options?: object) => computed(() => new Intl.NumberFormat(
    currentLocale.value,
  ...
  const relativeTime = (timestamp: number, formatter?: Intl.RelativeTimeFormat) => {
  ...
  const formattedCompactNumber = (number: number) => formattedNumber(
    number,
    compactNumberFormatter.value
  );

  const formattedPercentNumber = (number: number) => formattedNumber(
    number,
    percentNumberFormatter.value
  );
