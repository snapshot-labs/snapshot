...
const selectedItems = ref<{ id: number | string; name: string }[]>([
  props.items[0]
]);

watch(selectedItems, () => emit('select', selectedItems.value));
...
watch(
  ...
  () => {
    ...
    selectedItems.value = selected ? selected : [props.items[0]];
  },
  { immediate: true }
);
</script>
